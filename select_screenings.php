...
$id_web_movie = $query['id'];
$sql = mysqli_query($conn, "SELECT * FROM `movie` WHERE `id_movie` = '$id_web_movie'");
...
<header>
    ...
    <main>
        ...
        <br>
        <div class="head-name">
            <div class="line1"></div><span>CHỌN SUẤT CHIẾU</span>
            <div class="line2"></div>
        </div>
        <!--  -->
        <section class="detail">
            <div class="container booking">
                <figure class="detail-banner-booking">
                    <?php echo '<img src="./assets/img/poster/detail/' . $poster_movie . '" alt="Free guy poster" style="width:100%">'; ?>
                </figure>

                <div class="">
                <?php
                    $sql_day = mysqli_query($conn, "SELECT * FROM `screenings` WHERE `id_movie` = '$id_web_movie'");
                    $array_day = array();
                    while ($row = $sql_day->fetch_array(MYSQLI_ASSOC)) {
                        $id_movie = $row['id_movie'];
                        $day_playing = $row['day_playing'];
                        if (!in_array($day_playing, $array_day)) {
                            array_push($array_day, $day_playing);

                            echo
                                '<div class="detail-content-booking">
                                    <h4 class="title-cinema">' . $day_playing . '</h4>
                                    <div class="item-cinema">';

                            $sql_time = mysqli_query($conn, "SELECT * FROM `screenings` WHERE `id_movie` = '$id_web_movie' AND `day_playing` = '$day_playing'");
                            while ($row = $sql_time->fetch_array(MYSQLI_ASSOC)) {
                                $time_playing = $row['time_playing'];

                                echo
                                    '   <button><a href="./booking.php?id=' . $id_web_movie . '+' . $day_playing . '+' . $time_playing . '" style="text-decoration: none">' . $time_playing . '</a></button>';
                            }
                            echo '</div></div>';
                        }
                    }
